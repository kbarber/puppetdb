...
(defn update-catalog-metadata!
  ...
  [certname hash timestamp api-version catalog-version transaction-uuid]
  {:pre [(string? hash)
         (number? api-version)
         (string? catalog-version)
         ((some-fn nil? string?) transaction-uuid)]}
  (sql/update-or-insert-values :catalogs
                     ["certname=?" certname]
                     {:hash             hash
                      :timestamp        timestamp
...
(defn update-catalog-hash-match
  ...
  [certname hash {:keys [api-version version transaction-uuid]}]
  (inc! (:duplicate-catalog metrics))
  (update-catalog-metadata! certname hash timestamp api-version version transaction-uuid))
...
(defn update-catalog-hash-miss
  ...
  (let [catalog-id (:id (update-catalog-metadata! certname hash timestamp api-version
...
(defn add-catalog!
  ...
  ([certname catalog timestamp]
     (add-catalog! certname catalog timestamp nil))
  ([certname {:keys [api-version version transaction-uuid resources edges] :as catalog} timestamp catalog-hash-debug-dir]
             ...
             (let [resource-hashes (time! (:resource-hashes metrics)
               ...
               (sql/transaction
                (if (catalog-exists? hash)
                  (update-catalog-hash-match certname hash catalog timestamp)
                  (update-catalog-hash-miss certname hash catalog timetstamp resource-hashes catalog-hash-debug-dir)))
...
(defn replace-catalog!
  ...
  ([{:keys [certname] :as catalog} timestamp catalog-hash-debug-dir]
     ...
     (time! (:replace-catalog metrics)
            (sql/transaction
             (add-catalog! certname catalog timestamp catalog-hash-debug-dir)))))
