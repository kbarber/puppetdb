...
(defn get-response
  ([]      (get-response nil))
  ([query] (*app* (get-request "/v3/facts" query))))
    ...
    (testing "fact queries"
      (testing "well-formed queries"
        (doseq [[query result] {nil
          ...
          (let [request (get-request "/v3/facts" {"query" (json/generate-string query)})
      ...
      (testing "malformed, yo"
        (let [request (get-request "/v3/facts" {"query" (json/generate-string [])})
              {:keys [status body]} (*app* request)]
          (is (= status pl-http/status-bad-request))
          (is (= body "[] is not well-formed: queries must contain at least one operator"))))

      (testing "'not' with too many arguments"
        (let [request (get-request "/v3/facts" {"query" (json/generate-string ["not" ["=" "name" "ipaddress"] ["=" "name" "operatingsystem"]])})
              {:keys [status body]} (*app* request)]
          (is (= status pl-http/status-bad-request))
